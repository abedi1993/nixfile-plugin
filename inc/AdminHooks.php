...
class AdminHooks {
    ...
    public function register_hooks(): void {
        ...
        add_action( 'nixfile_daily_backup_event', [ $this, 'perform_daily_backup' ] );
        // Add cron job for immediate backup (first time activation)
        add_action( 'nixfile_immediate_backup_event', [ $this, 'perform_daily_backup' ] );
    ...
    public function set_daily_backup_rest( WP_REST_Request $request ): WP_REST_Response {
        ...
        $previous_state = $this->daily_backup;

        $this->daily_backup = $daily_backup;
        update_option( 'nixfile_uploader_daily_backup', $daily_backup );

        // Schedule or unschedule the cron job based on the setting
        if ($daily_backup) {
            $this->schedule_daily_backup();

            // If this is the first time activating, trigger an immediate backup
            if (!$previous_state) {
                // Run the backup in the background to avoid blocking the response
                wp_schedule_single_event(time(), 'nixfile_immediate_backup_event');
            }
        } else {
            $this->unschedule_daily_backup();
        }
    ...
    public function maybe_add_admin_bar_item(): void {
        ...
        $response = wp_remote_get( "https://api.nixfile.com/v1/upload-stats/?domain_id={$this->token}", [
                'headers' => [
    ...
    // Schedule the daily backup cron job when the setting is enabled
    private function schedule_daily_backup() {
        if (!wp_next_scheduled('nixfile_daily_backup_event')) {
            wp_schedule_event(time(), 'daily', 'nixfile_daily_backup_event');
        }
    }

    // Unschedule the daily backup cron job when the setting is disabled
    private function unschedule_daily_backup() {
    ...
    // Add this method to check if a backup is currently running
    public function is_backup_running(): bool {
        return get_option('nixfile_backup_in_progress', false);
    }

    // Perform the daily backup
    public function perform_daily_backup() {
        if (!$this->daily_backup || empty($this->token) || $this->is_backup_running()) {
            return;
        }

        // Mark backup as in progress
        update_option('nixfile_backup_in_progress', true);

        try {
            // Create a backup of the site
            $backup_file = $this->create_site_backup();

            if ($backup_file) {
                // Upload the backup to the API
                $this->upload_backup_to_api($backup_file);
            ...
            update_option('nixfile_backup_in_progress', false);
        }
    }

    // Create a backup of the site
    private function create_site_backup() {
        ...
        // Use WordPress built-in functions to create database backup
        $command = "mysqldump --single-transaction -h {$wpdb->dbhost} -u {$wpdb->dbuser} -p{$wpdb->dbpassword} {$wpdb->dbname} > {$db_backup_file}";

        // Try to execute the command
        @exec($command, $output, $return_var);

        // If mysqldump failed, try alternative method
        if ($return_var !== 0) {
            // Create a simple database backup using WordPress functions
        ...
        // Create a zip file containing the database and the wp-content directory
        $zip = new ZipArchive();
        if ($zip->open($backup_file, ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE) {
            ...
            $this->add_folder_to_zip(WP_CONTENT_DIR, $zip, 'wp-content');
    ...
    // Add a folder to a zip file recursively
    private function add_folder_to_zip($folder, &$zip, $zip_folder = '') {
        ...
        foreach ($files as $file) {
            ...
            if (is_dir($file_path)) {
                $this->add_folder_to_zip($file_path, $zip, $zip_path);
            } else {
    ...
    // Upload the backup to the API
    private function upload_backup_to_api($backup_file) {
        if (empty($this->token)) {
            return false;
        }
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'https://api.nixfile.com/v1/upload');
